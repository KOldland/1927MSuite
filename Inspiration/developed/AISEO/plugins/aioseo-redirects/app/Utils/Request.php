...
class Request {
	...
	public static function getProtectedPaths() {
		...
		$wpAdmin    = wp_parse_url( get_admin_url() );
		$wpLogin    = wp_parse_url( wp_login_url() );
		$wpJson     = wp_parse_url( get_rest_url() );
		$wpContent  = wp_parse_url( content_url() );
		$wpIncludes = wp_parse_url( includes_url() );

		return apply_filters( 'aioseo_redirects_protected_paths', [
			$wpAdmin['path'],
			$wpIncludes['path'],
			$wpContent['path'],
			$wpLogin['path'],
			$wpJson['path'] . ( ! empty( $wpJson['query'] ) ? '?' . $wpJson['query'] : '' )
		] );
	}
