...
class Redirects404Log extends CommonModels\Model {
	...
	public static function getFiltered( $offset = 0, $orderBy = 'last_accessed', $orderDir = 'DESC' ) {
		return aioseo()->db->start( 'aioseo_redirects_404_logs as `l1`' )
			->select( 'l1.url as id, l1.url, l2.hits, l1.created as last_accessed, l1.request_data, l1.ip' )
			->join(
				'(SELECT MAX(id) as id, count(*) as hits FROM ' . aioseo()->db->db->prefix . 'aioseo_redirects_404_logs GROUP BY `url`) as `l2`',
				'`l2`.`id` = `l1`.`id`',
				'',
				true
			)
