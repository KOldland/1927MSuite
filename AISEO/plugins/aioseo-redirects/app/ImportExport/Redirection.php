...
class Redirection extends Importer {
	...
	public function doImport() {
		if ( ! aioseo()->db->tableExists( 'redirection_items' ) ) {
			return;
		}

		$rules = aioseo()->db->start( 'redirection_items' )
		...
		foreach ( $rules as $rule ) {
			...
			$matchedUrl = Utils\Request::getMatchedUrl( $rule->url );
			$redirect->set( [
				'source_url'       => $rule->url,
				'source_url_match' => $matchedUrl,
				'target_url'       => Utils\Request::getTargetUrl( $rule->action_data ),
